...
import {
  startCapture, feedCaptureFrame, cancelCapture,
  startManualSample, stopManualSample,
  type CaptureState,
...
export function useCapture() {
  ...
  const [captureState, setCaptureState] = useState<CaptureState>({
    isRecording: false, label: "", samplesCount: 0, requiredSamples: 0,
    status: "idle", isSampleRecording: false, currentSampleFrames: 0,
  });
  ...
  const retrainModelAfterDelete = useCallback(async (type: SignType) => {
    const modelId = type === "letter" ? "rf-letter" : type === "word" ? "rf-word" : "rf-dynamic";
  ...
  const startLetterRecording = useCallback(async (letter: string) => {
    ...
    const state = startCapture("letter", letter, (count, extraState) => {
      setCaptureState((prev) => ({ ...prev, samplesCount: count, ...extraState }));
    }, () => {
      setCaptureState({ isRecording: false, label: "", samplesCount: 0, requiredSamples: 0, status: "done", isSampleRecording: false, currentSampleFrames: 0 });
  ...
  const startDynamicRecording = useCallback(async (sign: string) => {
    captureActiveRef.current = true;
  ...
  const triggerStartManualSample = useCallback(() => { startManualSample(); }, []);
  const triggerStopManualSample = useCallback(() => { stopManualSample(); }, []);

  const stopRecording = useCallback(async () => {
    cancelCapture();
    captureActiveRef.current = false;
    setCaptureState({ isRecording: false, label: "", samplesCount: 0, requiredSamples: 0, status: "idle", isSampleRecording: false, currentSampleFrames: 0 });
  }, []);
  ...
  return {
    ...
    startManualSample: triggerStartManualSample, stopManualSample: triggerStopManualSample,
    stopRecording, removeLetter, removeWord, removeDynamic,
    captureActiveRef,
  };
}
