...
export function startManualSample(): boolean {
  if (!currentCapture || currentCapture.type !== "dynamic") return false;
...
export async function finishCapture(): Promise<void> {
  ...
  try {
    ...
    console.log(`[Signum] Guardadas ${records.length} muestras de '${label}' con éxito.`);
  } catch (err) {
    console.error("[Signum] Error guardando muestras en SQLite:", err);
  }
